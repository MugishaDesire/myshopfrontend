...
export default function Home() {
  ...
  useEffect(() => {
    api.get("/products")
      ...
      .catch(() => { setError("Failed to load products. Please try again later."); setLoading(false); });
  }, []);

  // ── Close sidebar when screen grows (avoids ghost overlay) ─
  useEffect(() => {
  ...
  const addToCart = (product) => {
    ...
    setCart(prev => {
      ...
      return ex ? prev.map(i => i.id === product.id ? { ...i, quantity: i.quantity + 1 } : i) : [...prev, p];
    });
  ...
  const clearFilters = () => { setSelectedCategory("All"); setPriceRange([0, 5000]); setSearch(""); setSortBy("default"); };
  const hasActiveFilters = selectedCategory !== "All" || priceRange[0] > 0 || priceRange[1] < 5000 || search;

  // ── Filter panel JSX (shared between sidebar and drawer) ──
  const FilterPanel = ({ onClose }) => (
    <div className="filter-panel">
        ...
        <div className="cat-list">
          {CATEGORIES.map(cat => (
            <button
              ...
              onClick={() => { setSelectedCategory(cat); onClose && onClose(); }}
            >
              <span>
                {cat === "All" && "🏷️"}
                {cat === "Electronics" && "⚡"}
      ...
      <div className="filter-section">
        ...
        <div className="price-ticks"><span>$0</span><span>$2,500</span><span>$5,000</span></div>
      ...
      {hasActiveFilters && (
        <>
          <div className="filter-divider" />
          <button className="clear-all-btn" onClick={() => { clearFilters(); onClose && onClose(); }}>
            ✕ Clear All Filters
          </button>
        </>
      )}
    ...
    <>
      <div className="page-wrap">
        ...
        <div className="search-bar-wrap">
          <div className="search-bar-inner">
            ...
            <div className="search-meta">{filteredProducts.length}<span className="meta-of"> / {products.length}</span></div>
          </div>
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="modal-img-side">
                <div className={`modal-img-wrap ${imageZoom ? "zoomed" : ""}`} onClick={() => setImageZoom(!imageZoom)}>
                  <img
      ...
      <div className={`cart-drawer ${cartOpen ? "open" : ""}`}>
        <div className="cd-head">
          <div className="cd-head-title">🛒 Your Cart {totalItemsInCart > 0 && <span className="cd-head-badge">{totalItemsInCart}</span>}</div>
          <button className="cd-close" onClick={() => setCartOpen(false)}>✕</button>
        </div>
        {lastAdded && (
          <div className="cd-added-banner">
            <div className="cd-added-img">🛍️</div>
                ...
                <div className="cd-item" key={item.id}>
                  ...
                  <div className="cd-item-controls">
                    <div className="cd-qty-box">
                      <button className="cd-qty-btn" onClick={() => setCart(prev => prev.map(i => i.id === item.id ? { ...i, quantity: i.quantity - 1 } : i).filter(i => i.quantity > 0))}>−</button>
                      <span className="cd-qty-num">{item.quantity}</span>
                      <button className="cd-qty-btn" onClick={() => setCart(prev => prev.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i))}>+</button>
                    </div>
        ...
        {cart.length > 0 && (
          <div className="cd-footer">
            <div className="cd-totals">
              <div className="cd-row"><span className="cd-lbl">Subtotal</span><span>${cartTotal}</span></div>
              <div className="cd-row"><span className="cd-lbl">Shipping</span><span className={parseFloat(cartTotal) > 100 ? "cd-free" : ""}>{parseFloat(cartTotal) > 100 ? "FREE ✓" : "$9.99"}</span></div>
              <div className="cd-row cd-grand"><span>Total</span><span>${(parseFloat(cartTotal) + (parseFloat(cartTotal) > 100 ? 0 : 9.99)).toFixed(2)}</span></div>
            </div>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .filter-toggle-btn:not(.active):hover { background: var(--accent); color: white; border-color: var(--accent); }
        ...
        /* ── Category pills bar (mobile horizontal scroll) ── */
        .cat-pills-bar {
          display: none;
          gap: 6px; padding: 8px 16px 10px;
        ...
        .cat-btn { display: flex; align-items: center; justify-content: space-between; padding: 8px 10px; border-radius: 8px; background: none; border: 1.5px solid transparent; font-family: 'DM Sans', sans-serif; font-size: 0.88rem; font-weight: 500; color: var(--text); cursor: pointer; text-align: left; transition: all 0.15s; }
        ...
        .range-wrap { position: relative; height: 32px; margin-bottom: 4px; }
        .range-slider { position: absolute; width: 100%; left: 0; -webkit-appearance: none; appearance: none; height: 4px; background: transparent; outline: none; pointer-events: none; }
        .range-slider::-webkit-slider-thumb { -webkit-appearance: none; width: 20px; height: 20px; border-radius: 50%; background: var(--accent); border: 3px solid white; box-shadow: 0 2px 6px rgba(249,115,22,0.4); cursor: pointer; pointer-events: all; }
        ...
        .sort-select { width: 100%; padding: 8px 10px; border-radius: 8px; border: 1.5px solid var(--border); background: var(--bg); font-family: 'DM Sans', sans-serif; font-size: 0.88rem; font-weight: 500; color: var(--text); cursor: pointer; outline: none; }
        ...
        .badge.in-c  { top: 8px; right: 8px; background: var(--blue); color: white; }
        .heart-btn { position: absolute; bottom: 8px; right: 8px; background: rgba(255,255,255,0.92); border: none; border-radius: 50%; width: 30px; height: 30px; font-size: 0.9rem; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.12); transition: all 0.2s; z-index: 5; }
        ...
        .btn-cart, .btn-buy { flex: 1; padding: 9px 6px; border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 0.82rem; font-weight: 700; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 3px; min-height: 38px; }
        ...
        .cd-qty-btn { width: 26px; height: 26px; background: #f8fafc; border: none; font-size: 1rem; font-weight: 700; cursor: pointer; color: var(--text); transition: background 0.15s; display: flex; align-items: center; justify-content: center; }
        .cd-qty-btn:hover { background: var(--accent); color: white; }
        .cd-qty-num { width: 28px; text-align: center; font-weight: 700; font-size: 0.82rem; border-left: 1.5px solid var(--border); border-right: 1.5px solid var(--border); line-height: 26px; }
        ...
        /* Below 900px — switch to drawer filter */
        @media (max-width: 900px) {
          ...
          /* Show category pills bar */
          .cat-pills-bar { display: flex; }

          /* Hide "Filters" text label on small screens */
